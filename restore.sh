...
if [ -z "$OUTPUT_DIR" ]; then
  if [ -z "$REPOSTORE_OUTPUT_DIR" ]; then
    echo "Error, please export the output/target directory location with: 
  export REPOSTORE_OUTPUT_DIR=/dir/to/populate"
    exit 1
  else
    OUTPUT_DIR="$REPOSTORE_OUTPUT_DIR"
  fi
fi
if [ -z "$INPUT_FILE" ]; then
  if [ -z "$REPOSTORE_INPUT_FILE" ]; then
    echo "Error, please export the input csv file location with: 
  export REPOSTORE_INPUT_FILE=/dir/input_file.csv"
    exit 1
  else
    INPUT_FILE="$REPOSTORE_INPUT_FILE"
  fi
fi
